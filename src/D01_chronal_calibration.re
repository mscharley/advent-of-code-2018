let lines = Fs.readFileSync("./input/day1.txt")->Buffer.toString |> Js.String.split("\n") |> Js.Array.filter(n => n != "");
let inputs = lines->Belt.Array.map(n => {
  (n->String.sub(0, 1), int_of_string(n->String.sub(1, n->String.length - 1)))
});

let part2Cache = Js.Dict.empty();
part2Cache->Js.Dict.set("0", true);
let result = inputs->Belt.Array.reduce(0, (acc, (op, v)) => {
  let res = switch (op) {
    | "+" => acc + v
    | "-" => acc - v
    | _ => raise(Not_found)
...
while (!found^) {
  ...
  if (part2Cache->Js.Dict.get(acc^ ->string_of_int)->Belt.Option.getWithDefault(false)) {
    Js.log2("Part 2:", acc^);
    found := true;
  }
